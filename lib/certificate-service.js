...
class GreenlockService {
    ...
    async _internalAddDomain(domain) {
        try {
            // Only add domains which resolve
            await dns.resolve(domain, 'A');
        } catch (error) {
            logging.error(`Could not resolve domain ${domain}, marking failed`);
            ...
            logging.info(`CertificateJob: Set job status to FAILED for ${domain}`);
            return null;
        }
