...
class GreenlockService {
    constructor({greenlock, fastly, database}) {
        ...
        this.greenlock.events.on('cert_renewal', ({subject}) => {
        ...
        this.greenlock.events.on('cert_issue', async ({subject}) => {
            ...
            try {
                ...
                if (!certificateJob) {
                    throw new Error('Failed to find CertificateJob');
                }
                ...
                if (!certificateJob) {
                    logging.error(`No CertificateJob found for ${subject}`);
                } else {
    ...
    async getDomain(domain) {
        ...
        if (job.status === CertificateJobStatus.CREATED || job.status === CertificateJobStatus.DEPLOYED) {
            job.certificate = await this.greenlock.get({
                servername: domain
            });
        }
