 ...
 * - Deleted: Domain removed from cert-manager - kept only for observability
 */
...
class GreenlockService {
    constructor({greenlock, fastly, database}) {
        ...
        this.greenlock.events.on('cert_renewal', async ({subject}) => {
        ...
        this.greenlock.events.on('error', async (error) => {
            // This should only happen during renewal - new issues should be confirmed to point at Pro
            if (error.code === 'ENOTFOUND') {
                logging.error(`CertificateJob: DNS does not resolve for domain ${error.subject}`);
                await this._permanentFail(error.subject);
                ...
                await this._permanentFail(error.subject);
            } else if (error.message.includes('Error creating new order :: too many certificates')) {
                logging.error(`CertificateJob: Created too many certificates for domain ${error.subject}`);
                await this._permanentFail(error.subject);
            } else {
    ...
    async addDomain(domain) {
        ...
        const currentSite = await this.getDomain(domain);
        if (currentSite && currentSite.status !== CertificateJobStatus.DELETED) {
            logging.info(`CertificateJob: Job already exists for ${domain}`);
            return {
                // Site already exists
                message: `Domain ${domain} already exists in Greenlock`,
                success: true
            };
        }
    ...
    async removeDomain(domain) {
        await this.greenlock.manager.set({
            subject: domain,
            deletedAt: Date.now()
        });
        ...
        const [job, created] = await CertificateJob.findOrCreate({
            ...
            defaults: {
                status: CertificateJobStatus.DELETED
            }
        });
        if (!created) {
            job.status = CertificateJobStatus.DELETED;
            await job.save();
        }
        logging.info(`CertificateJob: Set job status to DELETED for ${domain}`);
    }
    ...
    async _uploadCertificate(certificateJob) {
        try {
            ...
            // TODO: This section is a bit hacky - issues seem to be within Greenlock itself,
            // so this won't prevent 100% of duplicate uploads. The only way to completely prevent
            // this (without fixing Greenlock) would be an UPLOADING state to ensure the job is
            // locked between instances too.
            await certificateJob.reload();
            if (certificateJob.status !== CertificateJobStatus.CREATED) {
                logging.info(`CertificateJob: Not uploading for ${certificateJob.subject}, status is ${certificateJob.status}`);
            }
    ...
    /**
     * Permanent failure for job - domain either no longer resolves, or does not resolve to Ghost Pro
     * @param {string} subject Removes subject from Greenlock
     */
    async _permanentFail(subject) {
