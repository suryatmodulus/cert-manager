...
class GreenlockService {
    constructor({greenlock, fastly, database}) {
        ...
        this.greenlock.events.on('cert_issue', async ({subject}) => {
            ...
            const certificateJob = await CertificateJob.findOne({
                where: {
                    subject
                }
            });
    ...
    async _init() {
        ...
        await this._testCreatedCertificates();
    ...
    async _queueFailedJobs() {
        ...
        for (const job of jobs) {
            ...
            if (certInfo) {
                job.status = CertificateJobStatus.CREATED;
            } else {
                job.status = CertificateJobStatus.QUEUED;
            }
    ...
    async _testCreatedCertificates() {
        ...
        jobs.forEach(async (certificateJob) => {
            try {
                // This may take up to 32 minutes - don't await the caller
